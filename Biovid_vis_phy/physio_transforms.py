...
def filter_emg(physio_df):
    ...
    physio_df=physio_df[(ts>=1000) & (ts<=5000)]
    emg_data=physio_df['emg_trapezius']
...
def get_spectrograms(emg_data):
    ...
    fs = 512  # Sampling frequency in Hz (adjust according to your data)
    nperseg = 16  # Number of data points per segment for spectrogram calculation
    noverlap = 8  # Overlap between segments
