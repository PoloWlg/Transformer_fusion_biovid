...
from models.models_seq import  VIS_PHY_MODEL, MultimodalTransformer, VIS_PHY_MODEL_CONCAT
...
def train(train_annotation,test_annotation,v_m_path, phy_m_path, weight_name):
    ...
    with experiment.train():
        ...
        for epoch in range(num_epochs):
            ...
            correct = 0
            total = 0
            for i,(spec_2d,video_batch, labels) in tqdm(enumerate(train_dataloader,0),total=len(train_dataloader), desc=f'Training'):
            ...
            if epoch % check_every == 0:
                ...
                if val_acc > best_val_acc:
                    ...
                    model_save_path_bb = os.path.join('/projets2/AS84330/Projets/MM_transformer/biovid_codes/Biovid_vis_phy',f'{weight_name}{round(best_val_acc,2)}_visphy.pth')
                    model_save_path_mmtransformer = os.path.join('/projets2/AS84330/Projets/MM_transformer/biovid_codes/Biovid_vis_phy',f'{weight_name}{round(best_val_acc,2)}.pth')
...
def test(test_annotation,concat_m_path, test_weight_vis_phy, test_weight_mm_transformer):

    b_size = 512
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    criterion = nn.CrossEntropyLoss()

    vis_phy_model=VIS_PHY_MODEL_CONCAT(concat_m_path).to(device=device)
    vis_phy_model = vis_phy_model.to(device=device)
    vis_phy_model_checkpoint_path = f'/projets2/AS84330/Projets/MM_transformer/biovid_codes/all_weights/best_weights_ca_transformers_84.6/{test_weight_vis_phy}'
    ...
    val_acc, _ = validate_mmtransformer_per_seq(vis_phy_model,mm_transformer, val_dataloader, criterion, device)
...
if __name__ == '__main__':
    ...
    for i in range (1,6):
        ...
        v_m_path = f'/projets2/AS84330/Projets/MM_transformer/biovid_codes/all_weights/weights_physio_viso4_fps5/model_best_vis_only_fold{i}.pth'
        phy_m_path = f'/projets2/AS84330/Projets/MM_transformer/biovid_codes/all_weights/weights_physio_viso4_fps5/model_best_gsr_fold{i}.pth'

        weight_name = f'model_best_feat_concat_fusion_mmtransformer_fold{i}_'
...
#     for i in range (1,6):
#         test_annotation = f'test_fold{i}.txt'
#         concat_m_path = f'/projets2/AS84330/Projets/MM_transformer/biovid_codes/all_weights/best_weights_feat_concat2/model_best_feature_concat_fold{i}.pth'

#         test_weight_vis_phy = f'model_best_feat_concat_fusion_mmtransformer_fold{i}_visphy.pth'
#         test_weight_mm_transformer = f'model_best_feat_concat_fusion_mmtransformer_fold{i}.pth'
#         best_accuracy = test(test_annotation,concat_m_path, test_weight_vis_phy, test_weight_mm_transformer)
#         kfold_accuracy.append(round(best_accuracy,1))
